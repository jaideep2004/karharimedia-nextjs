 ...
 *
 * Run: node server/src/scripts/fixBsonDepth.mjs
 *
 * This script:
 ...
 * 3. Sets deadLettered=false, state=queued, clears errorMessage so they can be retried
 *
  ...
  const cursor = db.collection('deliveryjobs').aggregate([
    ...
    },
    {
      $project: {
        ...
        state: 1,
        errorMessage: 1,
        cmSize: { $bsonSize: '$metadata.connectorMetadata' },
      }
    }
  ]);
...
async function fixBatch(db, ids) {
  ...
  for (const id of ids) {
    // Read current job's metadata to flatten connectorMetadata
    ...
    bulk.find({ _id: id }).updateOne({
      $set: {
        ...
        state: 'failed', // keep as failed — user will Fix & Retry manually
        deadLettered: false,
      },
      $unset: {
        errorMessage: '',
